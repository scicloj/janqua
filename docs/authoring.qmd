...
A `.qmd` file is a Quarto document, and Janqua adds live Jank evaluation on top — so Quarto's authoring features (markdown prose, math, callouts, frontmatter) work alongside `.jank` blocks. This chapter covers what you can do with those blocks — annotating values via the [Kindly](https://scicloj.github.io/kindly-noted/) convention, fence attributes, per-block options — and finishes with a few document-level Quarto features that pair well with live evaluation.

Kindly is a convention from the Clojure ecosystem for annotating values with rendering intent. Janqua supports Kindly [metadata](https://clojure.org/reference/metadata), so the same code patterns work in both Jank+Quarto and Clojure+Clay.
...
The Quarto-standard `echo` and `eval` attributes control display:
