...
If you combine Janqua with another Quarto filter that processes code blocks, list `jank` first in `filters:` — Janqua needs the `.jank` class to still be on the block when it runs.

Janqua emits HTML for charts and diagrams, so it's scoped to HTML output. Rendering to PDF, LaTeX, docx, or other formats won't run the filter — code blocks pass through untouched.
