...
Janqua is a [Pandoc Lua filter](https://pandoc.org/lua-filters.html). When Quarto renders a `.qmd` file, it calls [Pandoc](https://pandoc.org/) under the hood. Pandoc reads the document, builds an abstract syntax tree (AST), and then runs any filters specified in the frontmatter. The Janqua filter intercepts code blocks marked with the `.jank` class (typically written as `{.clojure .jank}` for editor syntax highlighting), evaluates them via a Jank [nREPL](https://nrepl.org/) server, and replaces them with the appropriate output.
...
| `:kind/mermaid` | String is rendered as a [Mermaid](https://mermaid.js.org/) diagram (client-side JS) |
| `:kind/graphviz` | String is rendered as a [Graphviz](https://graphviz.org/) diagram (client-side JS via [`viz-js`](https://github.com/mdaines/viz-js)) |
| `:kind/tex` | String is rendered as a LaTeX formula |
| `:kind/code` | String is displayed as syntax-highlighted Clojure code |
| `:kind/vega-lite` | Map is rendered as a [Vega-Lite](https://vega.github.io/vega-lite/) chart |
| `:kind/plotly` | Map is rendered as a [Plotly](https://plotly.com/javascript/) chart |
| `:kind/echarts` | Map is rendered as an [ECharts](https://echarts.apache.org/) chart |
| `:kind/cytoscape` | Map is rendered as a [Cytoscape](https://js.cytoscape.org/) graph |
| `:kind/highcharts` | Map is rendered as a [Highcharts](https://www.highcharts.com/) chart |
...
- **Graphviz**: The filter loads [`viz-js`](https://github.com/mdaines/viz-js) (a [WebAssembly](https://webassembly.org/) build of Graphviz) from CDN and calls `viz.renderSVGElement()` client-side. No local `dot` install needed.
