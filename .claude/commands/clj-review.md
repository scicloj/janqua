...
3. **Render GFM**: For each notebook, evaluate:
   ...
   (do (require '[dev :as dev] :reload)
       (dev/make-gfm! "janqua_book/<filename>.clj"))
   ```

4. **Run generated tests**: For each reviewed notebook, run only its generated
   test namespace. The mapping is:
   - Notebook: `notebooks/janqua_book/<name>.clj`
   - Test ns: `janqua_book.<name>-generated-test` (underscores → hyphens in ns)
   - Test file: `test/janqua_book/<name>_generated_test.clj`

   Run with:
   ```bash
   clojure -M:dev:test -m cognitect.test-runner -n janqua_book.<name>-generated-test
   ```
   If tests fail, diagnose the failure and suggest fixes.

5. **Read rendered markdown**: Read each generated file from `gfm/janqua_book.<name>.md`.
